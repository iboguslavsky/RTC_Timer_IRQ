...
struct Timer {
  Timer(uint16_t num_ticks, bool one_shot, timer_cb_t cb)
      : _num_ticks(roundUpToNearestFive(num_ticks)),
        _one_shot(one_shot),
        _cb(cb) {
    _capture_tick = 0;
    _running = false;
...
inline static void Timer::CMP_ovf_cb() {
  ...
  time_until_next_expiration =
      0xffff;  // largest delta between now and next timer expiration
  for (auto* timer : timers) {
...
inline void Timer::timerStart() {
  ...
  _capture_tick = roundUpToNearestFive(
      tick + _num_ticks);  // account for uint overflow when
                           // tick + _num_ticks > 65535
  _running = true;
...
inline static void Timer::init(bool highSpeed,
                               bool trueIRQ,
                               Stream& thisSerial) {
  _serial = &thisSerial;
  _trueIRQ = trueIRQ;
